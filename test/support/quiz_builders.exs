defmodule QuizBuilders do
  ...
  alias Mastery.Core.{Question, Template, Response, Quiz}
  ...
  def addition_checker(substitution, answer) do
    left = Keyword.fetch!(substitution, :left)
    right = Keyword.fetch!(substitution, :right)
  ...
  def build_question(overrides \\ []) do
    overrides |> template_fields |> Template.new() |> Question.new()
  end
